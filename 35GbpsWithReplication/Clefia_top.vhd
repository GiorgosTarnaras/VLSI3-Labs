...
architecture Behavioral of CLEFIA_128_CORE is
    ...
    -- SIGNALS
    -- =========================================================
    type state_type is (IDLE, GEN_L, SETUP, ROUNDS); -- FINISH removed
    ...
    signal current_rk_reg : std_logic_vector(127 downto 0); 
...
begin
    ...
    U_KEY_GEN: CLEFIA_Key_Row_Gen port map (
        L       => reg_L, con0 => CON(idx_k_con0), con1 => CON(idx_k_con1), 
        con2    => CON(idx_k_con2), con3 => CON(idx_k_con3),
        K       => reg_key, counter => active_exp_ctr, RK_Row => current_rk_row
    );
    ...
    -- =========================================================
    -- FSM PROCESS 1: Combinational Next-State Logic
    -- =========================================================
        ...
        case current_state is
            ...
            when ROUNDS =>
                if round_ctr = 17 then 
                    if start = '1' then
                        next_state <= GEN_L; -- Back-to-back pipeline jump
                    else
                        next_state <= IDLE;
                    end if;
                end if;
    ...
    -- =========================================================
    -- FSM PROCESS 2: Clocked Current State & Datapath
    -- =========================================================
    ...
    begin
        if rst = '1' then
            ...
            case current_state is
                ...
                when GEN_L =>
                    done <= '0'; -- Ensure done goes low after a back-to-back transfer
                    if ks_ctr = 11 then
                        ks_ctr <= 0;
                ...
                when ROUNDS =>
                    ...
                    if round_ctr = 17 then
                        ...
                        -- Instant load logic for continuous processing
                        if start = '1' then
                            reg_key <= key_in; reg_data <= data_in; mode_dec <= is_decrypt;
                            reg_L <= key_in; ks_ctr <= 0; round_ctr <= 0;
                        end if;
